...
def extract_titles(content_dir: Path) -> dict[str, tuple[str, str]]:
    ...
    For Hormozi Wiki, scan frameworks/ directory.
    """
    titles = {}

    dirpath = content_dir / "frameworks"
    ...
    for md_file in dirpath.glob("*.md"):
        ...
        # Check for either "title" or "name" field
        ...
        # For titles with long names like "Something: Description"
        # also register the short version before the colon
        if ":" in title:
            short = title.split(":")[0].strip()
            if len(short) > 3:
                # Only set short version if not already claimed
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Interlink Hormozi Wiki pages")
